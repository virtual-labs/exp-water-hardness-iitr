...
// Save and continue: called when first titration finishes (or when user presses Next)
function finishFirstTitration() {
  const experimentData = {
    normality_titrate: normality_titrate,
    volume_titrate: volume_titrate,
    vadded: vadded,
    result1: normality_titrant, // calculated normality of titrant
    volume_titrant: volume_titrant
  };

  try {
    localStorage.setItem('titrationExperiment_1', JSON.stringify(experimentData));
    console.log('Experiment 1 data saved to localStorage:', experimentData);
  } catch (e) {
    console.error('Failed to save experiment data to localStorage', e);
  }

  // Redirect to the next page (index2 shows intro video then goes to second titration)
...
function nextpressed() {
  console.log('Hi');
  // Ensure we save current readings and continue
  finishFirstTitration();
