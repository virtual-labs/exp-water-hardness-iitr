// Read first titration data from localStorage (preferred) or fallback to PHP-injected DOM
let var1, var2, var3, var4;
try {
  const exp1 = JSON.parse(localStorage.getItem('titrationExperiment_1')) || JSON.parse(localStorage.getItem('experiment1_data'));
  if (exp1) {
    var1 = exp1.normality_titrate; // Normality of titrate
    var2 = exp1.volume_titrate; // Volume of titrate
    var3 = exp1.vadded; // Volume added in first titration
    var4 = exp1.result1; // Calculated normality/result (maps to Result1)
  } else {
    // Fallback for older PHP flow where server injected values into DOM
    const data1 = document.getElementById('data1');
    if (data1) var1 = data1.getAttribute('data-php-variable');
    const data2 = document.getElementById('data2');
    if (data2) var2 = data2.getAttribute('data-php-variable');
    const data3 = document.getElementById('data3');
    if (data3) var3 = data3.getAttribute('data-php-variable');
    const data4 = document.getElementById('data4');
    if (data4) var4 = data4.getAttribute('data-php-variable');
  }
} catch (e) {
  console.error('Error reading experiment data from localStorage', e);
}

// Detect whether first-experiment data is available
const missingExp1 = (var4 === undefined || var4 === null || isNaN(parseFloat(var4)));
...
function setup() {
  ...
  if (missingExp1) {
    // If previous experiment data is missing, guide the user back to experiment 1
    startButton.textContent = 'Run Experiment 1';
    startButton.addEventListener('click', function(){
      window.location.href = 'Mytitration.html';
    });
    // show a visible warning in the UI
    const box = document.getElementById('textbox');
    if (box) box.innerText = 'No past titration data found. Click the button to run Experiment 1 first.';
  } else {
...
function addLiquidDrop() {
  if (liquidLevel >= 1 && bureteTouched == true) {
    ...
    // change colour of liquid on reaching the desired volume (only once)
    if (!titrationFinished && vadded >= volume_titrant) {
...
function nextpressed(){
  // Retrieve the first experiment data from localStorage (support both keys)
  const exp1Data = JSON.parse(localStorage.getItem('titrationExperiment_1')) || JSON.parse(localStorage.getItem('experiment1_data'));
  
  // Store the second experiment data
  const exp2Data = {
  ...
  localStorage.setItem('all_results', JSON.stringify(allResults));
  console.log('All results stored in localStorage:', allResults);

  // Navigate to the results page
  window.location.href = './TitrationComp.html';
}

// Create or show an HTML Next button anchored to the canvas container
function showHtmlNextButton(text, onClick) {
  try {
    let btn = document.getElementById('htmlNextBtn');
    if (!btn) {
    ...
    console.warn('Could not create HTML next button', e);
  }
}
